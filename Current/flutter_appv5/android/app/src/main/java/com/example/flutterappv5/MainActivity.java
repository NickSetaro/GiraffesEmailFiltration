                ...
                .setMethodCallHandler((call, result) -> {
                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                        ...
                        public void run() {
                                ...
                                else if(call.method.equals("removeNot")){
                                    String address = call.argument("address");
                                    String uuid = call.argument("uuid");
                                    user.removeNotFromBucket(address, uuid);
                                    result.success("Notification removed");
                                }

                                else if(call.method.equals("addKeyword")){
                                    String address = call.argument("email");
                                    String keyword = call.argument("keyWord");
                                    user.addKeyword(address, keyword);
                                    result.success("Keyword added");
                                }

                                else if(call.method.equals("getKeywords")){
                                    String address = call.argument("address");
                                    result.success(user.getKeywords(address));
                                }

                                else if(call.method.equals("deleteKeyWord")){
                                    String address = call.argument("address");
                                    String keyword = call.argument("keyword");
                                    user.removeKeyword(address, keyword);
                                    result.success("KeyWord removed");
                                }
