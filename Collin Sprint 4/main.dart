...
class _HomePageState extends State<HomePage> {
  ...
  Map<String, List<String>> map = {'September': ["work", "please"]};
  ...
  void setNotList(Bucket b) {
    Notif not;
    int index = 0;
    for(String s in notList){
      if(s == "/"){
        not = new Notif(b.address);
        index = notList.indexOf(s);
        not.subject = notList[index + 2];
        not.dateAndBody = notList[index + 1] + notList[index + 3] + " ";
